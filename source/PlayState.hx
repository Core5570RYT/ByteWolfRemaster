...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
			...
			case 'outside':
				{
					...
					bopshit = new FlxSprite(bg.x + 240, bg.y + 900);
		...
		if (FlxG.save.data.songPosition)
			{
				songPosBG = new FlxSprite(0, FlxG.height * 0.9 + 45).loadGraphic(Paths.image('healthBar'));
				...
				songName = new FlxText(songPosBG.x + (songPosBG.width / 2) - 20,songPosBG.y,0,"", 20);
	...
	override public function update(elapsed:Float)
	{
		...
		if (!offsetTesting)
			scoreTxt.text = Ratings.CalculateRanking(songScore,songScoreDef,nps,maxNPS,accuracy) + "\nHealth: " + healthPercent + "%" + "\nCombo: " + combo + "x" + " (" + "Max " + maxCombo + "x" + ")" + "\n" + "Song: " + SONG.song.toUpperCase() + " - " + storyDifficultyText.toUpperCase();
			else
		...
		for (i in 0...strumLineNotes.length)
			{
				...
				if (strumLineNotes.members[i].animation.curAnim.name == 'confirm' && !curStage.startsWith('school'))
					{
					}
				else
		...
		for (i in 0...strumLineNotes.length)
			{
				...
				strumLineNotes.members[i].x = (FlxMath.lerp(50 + (i * strumLineNotes.members[i].width), strumLineNotes.members[i].x, 0.95));	
			}
		...
		for (i in 0...playerStrums.length)
			{
				...
				playerStrums.members[i].x = (FlxMath.lerp(870 + (i * playerStrums.members[i].width), playerStrums.members[i].x, 0.95));	
			}
		...
		if (healthBar.percent < 20)
			{
				if (SONG.player2.toLowerCase() == "byte")
					{
...
/*				if (note.noteType == 2)
					...
					if (FlxG.save.data.botplay)
						{
							...
							//spawnNoteSplashOnNote(note); man, do not ever enable this option. it'll look very fucking bad lololol
						} else{
							playerStrums.forEach(function(spr:FlxSprite)
							{
								if (Math.abs(note.noteData) == spr.ID)
	...
	override function beatHit()
	{
		...
		if (camZooming && curSong.toLowerCase() == 'ok')
			{
				FlxG.camera.zoom += 0.015;
				camHUD.zoom += 0.04;
				camNOTES.zoom += 0.04;
				...
				switch(curBeat)
					{
						...
						case 256:
							healthBarMove();
							gfSpeed = 1;
							Application.current.window.title = "Friday Night Funkin': VSByteWolf (Not Responding)";
						case 320:
							inDropSong = false;
							gfSpeed = 2;
							healthBarBack();
							byteHasFinishedHisSong = !byteHasFinishedHisSong;
							Application.current.window.title = "Friday Night Funkin': VSByteWolf";
	...
	function doDisco()
		{
			...
			coolOverlay.alpha = 0.3;

			colorNum = FlxG.random.int(0,8);
			switch (colorNum)
				{
					case 0:
						coolOverlay.color = FlxColor.RED;
					case 1:
						coolOverlay.color = FlxColor.GREEN;
					case 2:
						coolOverlay.color = FlxColor.BLUE;
					case 3:
						coolOverlay.color = FlxColor.MAGENTA;
					case 4:
						coolOverlay.color = FlxColor.ORANGE;
					case 5:
						coolOverlay.color = FlxColor.CYAN;
					case 6:
						coolOverlay.color = FlxColor.BROWN;
					case 7:
						coolOverlay.color = FlxColor.LIME;
					case 8:
						coolOverlay.color = FlxColor.PURPLE;
